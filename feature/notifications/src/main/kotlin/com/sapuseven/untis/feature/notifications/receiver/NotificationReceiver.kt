package com.sapuseven.untis.feature.notifications.receiver
...
class NotificationReceiver : BroadcastReceiver() {
	...
	override fun onReceive(context: Context, intent: Intent) = runBlocking {
		...
		if (intent.getBooleanExtra(EXTRA_BOOLEAN_CLEAR, false)) {
			Log.d(LOG_TAG, "Attempting to cancel notification #${intent.getIntExtra(EXTRA_INT_ID, -1)}")
	...
	private fun sendMessagesNotification(context: Context, messages: List<MessageOfDay>) {
		val builder = NotificationCompat.Builder(context, CHANNEL_ID_FIRSTLESSON)
			.setContentTitle(context.getString(R.string.feature_notifications_text_motd_title))
			.setContentText(
				context.getString(
					R.string.feature_notifications_text_motd_message,
					messages.size
				)
			)
	...
	private fun sendBreakNotification(
		...
		val title = context.getString(
			if (isFirst) R.string.feature_notifications_text_first_title else R.string.feature_notifications_text_title,
			intent.getStringExtra(EXTRA_STRING_BREAK_END_TIME)
		)
		val message = buildMessage(
			null,
			intent,
			context.getString(R.string.feature_notifications_text_message_separator),
		...
		val deepLinkUri = "untis://timetable/period/TODO".toUri() // TODO Implement deep link handling
		...
		val builder = NotificationCompat.Builder(context, CHANNEL_ID_BREAKINFO)
			...
			.setSmallIcon(R.drawable.feature_notifications_clock)
	...
	private fun buildMessage(
		...
		if (intent.getStringExtra(EXTRA_STRING_NEXT_SUBJECT)?.isBlank() != false) null else
			when (visibilitySubjects) {
				NotificationVisibility.SHORT -> context?.getString(
					R.string.feature_notifications_text_message_subjects,
		...
		if (intent.getStringExtra(EXTRA_STRING_NEXT_ROOM)?.isBlank() != false) null else
			when (visibilityRooms) {
				NotificationVisibility.SHORT -> context?.getString(
					R.string.feature_notifications_text_message_rooms,
		...
		if (intent.getStringExtra(EXTRA_STRING_NEXT_TEACHER)?.isBlank() != false) null else
			when (visibilityTeachers) {
				NotificationVisibility.SHORT -> context?.getString(
					R.string.feature_notifications_text_message_teachers,
		...
		if (intent.getStringExtra(EXTRA_STRING_NEXT_CLASS)?.isBlank() != false) null else
			when (visibilityClasses) {
				NotificationVisibility.SHORT -> context?.getString(
					R.string.feature_notifications_text_message_classes,
